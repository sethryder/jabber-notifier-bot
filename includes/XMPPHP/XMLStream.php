...
class XMPPHP_XMLStream {
	 ...
	 * @var integer
	*/
	protected $whitespacePing = 30;
	/**
	 * @var integer
	*/
	protected $lastwhitespacePing = 0;
	...
	private function __process($maximum=5) {
		...
		do {
			...
			if (time() - $this->lastwhitespacePing > $this->whitespacePing) {
				$this->log->log("Sending whitespace ping",  XMPPHP_Log::LEVEL_DEBUG);
				$this->send(' ');
				$this->lastwhitespacePing = time();
			}
	 ...
	 * @return string
	 */
	public function processUntil($event, $timeout=10) {
		$start = time();
